...
class Converter:
    ...
    def convert_template(self, template_data):
        ...
        variable-substitution → preprocess_html → html2text → token restore — no
        link map and none of convert_page's ## Comments / ## Attachments sections.
        ...
        md = self.html_to_markdown(processed)
        # Restore weber sentinel tokens (panels → ::: callouts, TOC, draw.io
        # mermaid) the same way convert_page does, else raw WCEXTEND tokens leak.
        return self._restore_md_tokens(md)
...
def import_templates(space_key, dry_run, debug, log_file):
    ...
    converter = Converter(strip_patterns=strip_patterns_from_env(),
                          **weber_config_from_env())
